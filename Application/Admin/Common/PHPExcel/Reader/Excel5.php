...
//		$recordData = substr($this->_data, $this->_pos + 4, $length);
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset: 16; size: 8; header margin (IE4E6E9 754 floating-point value)
			$marginHeader = self::_extractNumber(substr($recordData, 16, 8));
			$this->_phpSheet->getPageMargins()->setHeader($marginHeader);

			// offset: 24; size: 8; footer margin (IE4E6E9 754 floating-point value)
	...
	private function _readRk()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			$numValue = self::_GetIE4E6E9754($rknum);
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			...
			if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
				...
				$numValue = self::_GetIE4E6E9754(self::_GetInt4d($recordData, $offset + 2));
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				if (!$this->_readDataOnly) {
					// add style
					$cell->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
...
//			var_dump($bcBitCount);
	...
	private static function _readBIFF8Constant($valueData)
	{
		...
		case 0x00: // empty constant (what is this?)
			...
			break;
		case 0x01: // number
			// offset: 1; size: 8; IE4E6E9 754 floating-point value
	...
	/**
	 * Reads first 8 bytes of a string and return IE4E6E9 754 float
	 *
	...
	private static function _GetIE4E6E9754($rknum)
	{
		if (($rknum & 0x02) != 0) {
			$value = $rknum >> 2;
		} else {
			// changes by mmp, info on IE4E6E9754 encoding from
			// research.microsoft.com/~hollasch/cgindex/coding/iE4E6E9float.html
